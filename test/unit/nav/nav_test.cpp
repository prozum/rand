...
void NavTest::initNav() {
    ...
    init_rep(&RepStruct, &FCStruct, &LaserStruct, &SonarStruct, &IrTopStruct,
             &IrBottomStruct);
}

void NavTest::alignToGrid_bottomOfRoom_expectBottomEdgeOfMap() {
    const uint16_t xCoord =
                       MAP_MIDDLE - (MAP_HEIGHT * CENTIMETERS_PR_PIXEL / 2),
                   yCoord = MAP_MIDDLE;
...
void NavTest::alignToGrid_leftInRoom_expectLeftEdgeOfMap() {
    const uint16_t xCoord = MAP_MIDDLE,
                   yCoord = MAP_MIDDLE - (MAP_WIDTH * CENTIMETERS_PR_PIXEL / 2);
...
void NavTest::alignToGrid_outOfBoundsBottom_expectWarningAndY0() {
    const uint16_t xCoord = MAP_MIDDLE,
                   yCoord = MAP_MIDDLE -
                            ((MAP_WIDTH + 10) * CENTIMETERS_PR_PIXEL / 2);
...
void NavTest::alignToGrid_outOfBoundsLeft_expectWarningAndX0() {
    const uint16_t xCoord =
                       MAP_MIDDLE - (MAP_WIDTH + 10) * CENTIMETERS_PR_PIXEL / 2,
                   yCoord = MAP_MIDDLE;
...
void NavTest::alignToGrid_outOfBoundsRight_expectWarningAndX64() {
    const uint16_t xCoord = MAP_MIDDLE +
                            (MAP_HEIGHT + 10) * CENTIMETERS_PR_PIXEL / 2,
                   yCoord = MAP_HEIGHT;
...
void NavTest::calculateXDistance_ang0Dist10_expect10() {
    ...
    std::string msg = "Converted to floats: expected: " +
                      std::to_string(fix16_to_float(expected)) + ", result: " +
...
void NavTest::calculateXDistance_ang90Dist10_expect0() {
    ...
    std::string msg = "Converted to floats: expected: " +
                      std::to_string(fix16_to_float(expected)) + ", result: " +
                      std::to_string(fix16_to_float(result));
    CPPUNIT_ASSERT_EQUAL_MESSAGE(msg, (int)(conv_expected * 10),
                                 (int)(conv_result * 10));
}
