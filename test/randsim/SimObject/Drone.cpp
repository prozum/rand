...
void Drone::updateRotation(uint16_t yaw_value) {
    ...
    double rotationVelocity = -calculateVelocity(yaw_value, ROTATION_SPEED / PERIODS_PER_SEC);
...
void Drone::updateStrafe(uint16_t roll_value) {
    double strafeVelocity = calculateVelocity(roll_value, STRAFE_SPEED / PERIODS_PER_SEC);
...
void Drone::updateFrontal(uint16_t pitch_value) {
    ...
    double moveVelocity = -calculateVelocity(pitch_value, MOVEMENT_SPEED / PERIODS_PER_SEC);
...
void Drone::updateHeight(uint16_t throttle_value) {
    ...
    double altitudeVelocity = calculateVelocity(throttle_value, ALTITUDE_SPEED / PERIODS_PER_SEC);
