...
void Minimap::printMap() { map_show(); }
...
void Minimap::draw() {
    ...
    for (int X = 0; X < MAP_WIDTH; ++X) {
        for (int Y = 0; Y < MAP_HEIGHT; ++Y) {
            ...
            case VISITED:
                ...
                Sim.Render->drawRect({X * BlockSize.X, Y * BlockSize.Y},
                                     BlockSize);
                break;
            case WALL:
    ...
    Sim.Render->drawRect(
        {Sim.Drn->NavStruct.posx / CentPerBlock,
         MapHeightBlock - Sim.Drn->NavStruct.posy / CentPerBlock},
        BlockSize);
