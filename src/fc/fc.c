...
void rotate_left(fc_t *fc) { fc->yaw = fc->duty->MIN_FC_DUTY; }

void rotate_right(fc_t *fc) { fc->yaw = fc->duty->MAX_FC_DUTY; }

void rotate_stop(fc_t *fc) { fc->yaw = fc->duty->MID_FC_DUTY; }

void move_left(fc_t *fc) { fc->roll = fc->duty->MIN_FC_DUTY; }

void move_right(fc_t *fc) { fc->roll = fc->duty->MAX_FC_DUTY; }

void move_forward(fc_t *fc) { fc->pitch = fc->duty->MAX_FC_DUTY; }

void move_back(fc_t *fc) { fc->pitch = fc->duty->MIN_FC_DUTY; }

void move_up(fc_t *fc) { fc->throttle = fc->duty->MAX_FC_DUTY; }

void move_down(fc_t *fc) { fc->throttle = fc->duty->MIN_FC_DUTY; }
...
acceleration_t fc_read_acceleration(fc_t *fc) {
    ...
    acceleration_t accel = {fix16_from_float(0), fix16_from_float(0.5), fix16_from_float(0)};
    return accel;
}