 ...
 */
typedef enum reg_e { PORTB_, PORTC_, PORTD_ } reg_t;
 ...
 */
static uint8_t dpins[] = {_BV(PORTD0), _BV(PORTD1), _BV(PORTD2), _BV(PORTD3), _BV(PORTD4), _BV(PORTD5), _BV(PORTD6),
 ...
 */
typedef enum dpin_e { P0, P1, P2, P3, P4, P5, P6, P7, P8, P9, P10, P11, P12, P13 } dpin_t;
 ...
 */
typedef enum apin_e { A0, A1, A2, A3, A4, A5 } apin_t;

/**
 * An enum for storing the physical serial receive pins on the board
 */
typedef enum rx_e { RX0 = 0, USB_RX = 0xFF } rx_t;

/**
 * An enum for storing the physical serial transmit pins on the board
 */
typedef enum tx_e { TX1 = 1, USB_TX = 0xFF } tx_t;

/**
 * An enum for storing the physical pins that support PWM
 */
typedef enum pwm_e { PWM3 = 3, PWM5 = 5, PWM6 = 6, PWM9 = 9, PWM10 = 10, PWM11 = 11 } pwm_t;
