...
const uint16_t port_to_input[] = {
    NOT_A_PORT,      NOT_A_PORT, (uint16_t)&PINB, NOT_A_PORT,      NOT_A_PORT,
    (uint16_t)&PINC, NOT_A_PORT, NOT_A_PORT,      (uint16_t)&PIND,
};
...
void adc_init() {
    ADCSRA |=
        ((1 << ADPS2) | (1 << ADPS1) | (1 << ADPS0)); // Set prescaler bits in ADCSRA(ADC control and status register A)
