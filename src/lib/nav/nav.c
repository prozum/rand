...
void navigation(rep_t *rep, nav_t *nav) {
    ...
    case TURNING:
        ...
        break;
    case MOVEFORWARD:
        on_move_forward(rep, nav);
        break;
    case MOVEUP:
        on_move_up(rep, nav);
        break;
    case MOVEDOWN:
        on_move_down(rep, nav);
        break;
    case FOLLOW_FORWARD:
        on_follow_forward(rep, nav);
        break;
    case FOLLOW_FURTHER:
        on_follow_further(rep, nav);
        break;
    case FOLLOW_CHECK:
        on_follow_check(rep, nav);
        break;
    case FOLLOW_TURN:
        on_follow_turn(rep, nav);
        break;
    case SEARCHING:
        on_searching(rep, nav);
        break;
    default:
        printf("Invalid task!");
        break;
...
void align_to_wall(rep_t *rep, nav_t *nav) {
    ...
    degrees_to_turn = diff_wall; // fix16_rad_to_deg(fix16_asin(fix16_div(drift,
                                 // fix16_mul(fix16_sin(fix16_from_int(PERPENDICULAR_RAD)), diffWall)))); todo: Insert
                                 // proper calculation
...
void map_set_point(uint8_t x, uint8_t y, fieldstate_t field) { map_write(x, y, field); }
...
fieldstate_t map_check_point(uint8_t x, uint8_t y) { return map_read(x, y); }
...
uint8_t is_sonar_reliable(rep_t *rep, state_t state) {
    ...
    if (fix16_abs(fix16_sub(calc_sonar_dist_to_wall, fix16_from_int(rep->sonar->value)) <=
                  fix16_from_int(SENSOR_DEVIATION)) &&
        rep->sonar->valid) {
        return 1;
    } else {
        return 0;
