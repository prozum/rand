    ...
    for (i = 0; i < left_matrix->rows; ++i) {
        for (j = 0; j < right_matrix->columns; ++j) {
            ...
            for (k = 0; k < left_matrix->columns; ++k) {
                result = result + matrix_get(left_matrix, i,k) * matrix_get(right_matrix, k,j);
            }
...
matrix_t *mult_const_vec(matrix_t *vector, float k) {
    ...
    if(vector->rows > 1) {
        for (i = 0; i < vector->rows; ++i) {
            matrix_set(res, i, 0, k * matrix_get(vector, i, 0));
        }
    }
    else {
        for (i = 0; i < vector->columns; ++i) {
            matrix_set(res, 0, i, k * matrix_get(vector, 0, i));
        }
    }
    return res;
}
...
matrix_t *mult_mat_vec(matrix_t *matrix, matrix_t *vector) {
    ...
    for (i = 0; i < matrix->rows; ++i) {
        ...
        for (j = 0; j < matrix->columns; ++j) {
            matrix_set(res, i,0, matrix_get(res, i, 0) + matrix_get(matrix, i, j) * matrix_get(vector, j, 0));
        }
    }
    return res;
}
...
matrix_t *add_mat_mat(matrix_t *left, matrix_t *right) {
    ...
    for (i = 0; i < left->rows; ++i) {
        for (j = 0; j < right->columns; ++j) {
            matrix_set(res, i, j, matrix_get(left, i, j) + matrix_get(right, i, j));
        }
    }
    return res;
}
...
matrix_t *add_vec_vec(matrix_t *left, matrix_t *right) {
    ...
    if(left->rows > 1) {
        for (i = 0; i < left->rows; ++i) {
            matrix_set(res, i, 0, matrix_get(left, i, 0) + matrix_get(right, i, 0));
        }
    } else {
        for (i = 0; i < left->columns; ++i) {
            matrix_set(res, 0, i, matrix_get(left, 0, i) + matrix_get(right, 0, i));
        }
    }
...
matrix_t *inv_mat(matrix_t *matrix) {
    ...
    det = matrix_get(matrix, 0, 0) * matrix_get(matrix, 1, 1) - matrix_get(matrix, 1, 0) * matrix_get(matrix, 0, 1);
    //Allocate space
    matrix_t *inv_mat = matrix_constructor(matrix->rows, matrix->columns);

    for (i = 0; i < SIZE; ++i) {
        for (j = 0; j < SIZE; ++j) {
            matrix_set(inv_mat, i, j, matrix_get(matrix, i, j) / det);
            if (j != i)
                matrix_set(inv_mat, i, j, -matrix_get(inv_mat, i, j));
        }
    }
    float temp = matrix_get(inv_mat, 0, 0);
...
matrix_t *ident_mat(uint8_t size) {
    ...
    for(i = 0; i < size; i++) {
        for(j = 0; j < size; j++) {
            if(i == j)
                matrix_set(res, i, j, 1);
            else
                matrix_set(res, i, j, 0);
        }
    }
...
matrix_t *matrix_constructor(uint8_t rows, uint8_t columns) {
    ...
    new_matrix->values = malloc(columns * rows * sizeof(float));
